...
module.exports.solution = (bridge_length, weight, truck_weights) => {
  ...
  while (countTrucks > passedTrucks.length) {
    ...
    passingBridgeTrucks = passingBridgeTrucks.filter(truck => truck.time < bridge_length);

    // 현재 다리를 지나고 있는 트럭의 무게를 확인합니다.
    ...
    passingBridgeTrucks = passingBridgeTrucks.map((truck) => {
      truck.time += 1;
      return truck;
    });
